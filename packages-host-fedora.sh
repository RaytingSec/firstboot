...
packages=(
    ...
    "python3-test"  # Needed for py_vollib, else `_testcapi` error
...
cat /tmp/init/linux-config/ipython_config.py | tee -a ~/.ipython/profile_default/ipython_config.py
...
# cp /tmp/init/linux-config/vlc-qt-interface.conf ~/.config/vlc
...
# cat /tmp/init/linux-config/dnf.conf | sudo -E tee -a /etc/dnf/dnf.conf > /dev/null
